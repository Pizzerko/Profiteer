...
import type { HistoryResponse, NewsItem, Portfolio, Quote } from "../api/types";
import { money, pct, plClass } from "../utils/format";
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-5">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <label
              className={`ml-auto flex items-center gap-1.5 text-xs ${
                INTRADAY.has(range) ? "text-slate-300" : "text-slate-600"
              }`}
              title="Include pre-market and after-hours (intraday ranges only)"
            >
              <input
                type="checkbox"
                checked={prepost}
                disabled={!INTRADAY.has(range)}
          ...
          <StockChart points={history?.points ?? []} up={up} />
        </div>
